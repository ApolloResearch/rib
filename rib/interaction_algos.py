...
from rib.data_accumulator import (
    collect_Lambda_dash,
    collect_M_dash,
    collect_M_dash_and_Lambda_dash,
)
...
def calculate_interaction_rotations_local(
...
def calculate_interaction_rotations_global(
    gram_matrices: dict[str, Float[Tensor, "d_hidden d_hidden"]],
    section_names: list[str],
    node_layers: list[str],
    hooked_model: HookedModel,
    ...
    """Calculate the interaction rotation matrices (denoted C) and their psuedo-inverses for a
    global interaction basis like gradient flow.

    This function implements Algorithm 4(?) of the paper (TODO)
    ...
    Args:
        ...
        rotate_final_node_layer: Whether to rotate the final layer to its eigenbasis (which is
            equivalent to its interaction basis). Defaults to True.
        basis_formula: The formula to use for the integrated gradient. Must be "g*f" for now.
            This is gradient flow. Defaults to "g*f".
        svd_basis: Returns Us as Cs, so basis is just the eigenvectors of the gram matrix.
    ...
    module_names = section_names_to_calculate
    hook_names = module_names

    M_matrices = collect_M_dash(
        hooked_model=hooked_model,
        section_names=section_names_to_calculate,
    ...
    for node_layer, section_name in tqdm(
        ...
        # now calculate Lambda_dash
        C_out = Cs[-1].C.to(device=device) if Cs[-1].C is not None else None
        Lambda_dash = collect_Lambda_dash(
            C_out=C_out,
            hooked_model=hooked_model,
            n_intervals=n_intervals,
            data_loader=data_loader,
            module_name=section_name,  # TODO: check if this is correct. Should it be node_layer?
            dtype=dtype,
            device=device,
            hook_name=node_layer,
            Lambda_einsum_dtype=Lambda_einsum_dtype,
...
LOCAL_BASIS_FORMULAE = ["(1-alpha)^2", "(1-0)*alpha", "svd"]
GLOBAL_BASIS_FORMULAE = ["g*f"]
...
def calculate_interaction_rotations(
    ...
    truncation_threshold: float = 1e-5,
    rotate_final_node_layer: bool = True,
    basis_formula: Literal["(1-alpha)^2", "(1-0)*alpha", "svd", "g*f"] = "(1-0)*alpha",
) -> tuple[list[InteractionRotation], list[Eigenvectors]]:
    """
    Calls either calculate_interaction_rotations_local or calculate_interaction_rotations_global
    depending on whether the basis is local or global.
    """
    if basis_formula in LOCAL_BASIS_FORMULAE:
        return calculate_interaction_rotations_local(
    ...
    elif basis_formula in GLOBAL_BASIS_FORMULAE:
        return calculate_interaction_rotations_global(
            gram_matrices=gram_matrices,
            section_names=section_names,
            node_layers=node_layers,
            hooked_model=hooked_model,
        ...
        raise ValueError(f"Unrecognized basis formula {basis_formula}")
