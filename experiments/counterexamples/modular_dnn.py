...
def main(config: Config) -> None:
    ...
    parent_dir = Path(__file__).parent / "results"
    out_dir = parent_dir / exp_name #/ "rib_graph.pt"
    ...
    torch.save(results, out_dir / "results_new_nov_.pt")
    logger.info("Saved results to %s", out_dir)

    # Plot the graphs
    out_file_graph = out_dir / "rib_graph_new_nov_A.png"
    out_file_neuron_basis = out_dir / "neuron_basis_graph_new_nov_A.png"
    out_file_pca = out_dir / "pca_graph_new_nov_A.png"
    ...
    if not overwrite_output(out_file_graph, config.force):
        return
