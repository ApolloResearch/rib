...
function run_modularity_pipeline {
    ...
    cd "${interp_dir}"
    python generate_labels_mod_add.py "${build_dir}/out/${prefix}${type}${suffix}_rib_graph.pt" --out_file="${prefix}${type}${suffix}.csv" --max_dim 130 -f

    cd "${build_dir}"
    OMP_NUM_THREADS=1 python run_modularity.py "${build_dir}/out/${prefix}${type}${suffix}_rib_graph.pt" --ablation_path "${ablation_dir}/out/${prefix}${type}${suffix}_edge_ablation_results.json" --labels_file "${interp_dir}/${prefix}${type}${suffix}.csv" --gamma 1 --nodes_per_layer 30 --seed 0 --hide_const_edges True --plot_norm abs --figsize 10,14 --sorting cluster --hide_singleton_nodes True
    # --target_n_clusters $n_clusters
    # --line_width_factor 0.0002

    # Do not hide singletons because they may have small edges. Use sqrt norm to make visible.
    cd "${build_dir}"
    OMP_NUM_THREADS=1 python run_modularity.py "${build_dir}/out/${prefix}${type}${suffix}_rib_graph.pt" --ablation_path "${ablation_dir}/out/${prefix}${type}${suffix}_edge_ablation_results.json" --labels_file "${interp_dir}/${prefix}${type}${suffix}.csv" --gamma 1 --nodes_per_layer 130 --seed 0 --hide_const_edges True --plot_norm sqrt --figsize 10,30 --sorting clustered_rib --hide_singleton_nodes False --hide_output_layer False
...
run_modularity_pipeline ngf_rib 0
